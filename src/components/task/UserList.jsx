...
const UserList = ({ setTeam, setItem, member, team }) => {
  ...
  useEffect(() => {
    if (member) {
      setSelectedUsers(team?.filter((item) => item.id === member.id)[0]);
    } else {
      setSelectedUsers(team[0]);
    }
  }, [member, team]);
      ...
      >
        <div className="relative mt-1">
          <Listbox.Button className="relative w-full cursor-default rounded bg-white pl-3 pr-10 text-left px-3 py-2.5 2xl:py-3 border border-gray-300 sm:text-sm">
            <span className="block truncate">{selectedUsers?.name}</span>
